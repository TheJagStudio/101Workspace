        ...
        <div className="h-16 bg-white border-b border-gray-200 shadow-lg flex items-center justify-between px-6 z-50">
            ...
            <div className="flex items-center w-full sm:w-86 ml-5">
                <div className="relative w-full">
                    ...
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 peer-focus:text-green-500 text-xs bg-gray-100 peer-focus:bg-green-50 px-2 py-0.5 rounded">⌘ + K</span>
                    ...
                    {results?.length > 0 && (
                        <div className="absolute hidden peer-focus:block hover:block left-0 right-0 mt-2 py-2 bg-white border border-gray-200 rounded-xl shadow-lg z-50 h-fit w-full sm:w-[150%] overflow-hidden">
                            {results.map((item, idx) => (
                                <div
                                    key={item?.id || idx}
                                    className="flex flex-row flex-nowrap gap-2 w-full items-center justify-start px-4 py-1 h-12 hover:bg-green-50 cursor-pointer text-sm"
                                >
                ...
                {user?.first_name && (
                    <div className="relative">
                        <button className="flex items-center gap-3" onClick={() => setShowProfileMenu(!showProfileMenu)}>
                            <div className="w-9 h-9 rounded-full bg-green-100 flex items-center justify-center border border-gray-200">
                                <img
