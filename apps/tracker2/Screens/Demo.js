...
TaskManager.defineTask(LOCATION_TASK_NAME, async ({ data, error }) => {
	...
	if (data) {
		...
		if (location) {
			...
			// Fetch the session from storage to get the user ID
			const sessionStr = await AsyncStorage.getItem("supabase.auth.token");
			if (!sessionStr) {
				console.error("Background Task: No user session found.");
				return;
			}
			try {
				fetch("http://thejagstudio-ntfy.hf.space/location", {
					method: "POST",
					headers: {
				...
				console.error("Error sending location to ntfy:", error);
			}
			...
			if (!userId) {
				console.error("Background Task: User ID not found in session.");
				return;
			}

			// Insert location data directly into Supabase
			const { data, error } = await supabase.from("tracker_salesman").update({ current_location_lat: location.coords.latitude, current_location_lng: location.coords.longitude }).eq("authId", userId).select();
