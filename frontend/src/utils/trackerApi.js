...
const TRACKER_API_BASE = `${import.meta.env.VITE_SERVER_URL}/api/tracker`;
...
export const fetchAdminSettings = async () => {
    try {
        const response = await apiRequest(`${TRACKER_API_BASE}/admin-settings/`);
        return response;
    } catch (error) {
...
export const updateAdminSettings = async (data) => {
    try {
        const response = await apiRequest(`${TRACKER_API_BASE}/admin-settings/1/`, {
            method: 'PATCH',
            body: JSON.stringify(data),
        });
        return response;
